...
Form *			Intern::makeForm(std::string formName, std::string target) const
{
	...
	{
		std::cout << "Inter creates a Shrubbery Creation Form" << std::endl;
		return new ShrubberyCreationForm(target);
	}
	else if (formName == "robotomy request")
	{
		std::cout << "Inter creates a Robotomy Request Form" << std::endl;
		return new RobotomyRequestForm(target);
	}
	else if (formName == "presidential pardon")
	{
		std::cout << "Inter creates a Presidential Pardon Form" << std::endl;
